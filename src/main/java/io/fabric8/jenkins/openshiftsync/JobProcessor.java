...
public class JobProcessor extends NotReallyRoleSensitiveCallable<Void, Exception> {
    ...
    private Map<String, ParameterDefinition> createOrUpdateJob(Jenkins activeInstance, ItemGroup parent, String jobName,
        ...
        String existingBuildRunPolicy = populateBCProjectProperty(job, null, buildConfigProjectProperty);
        ...
        String runPolicy = buildConfig.getSpec().getRunPolicy();
        ...
        String rv = buildConfig.getMetadata().getResourceVersion();
        NamespaceName ns = NamespaceName.create(buildConfig);
        if (newJob) {
            try {
                ...
                logger.info("Created job " + jobName + " from BuildConfig " + ns + " with revision: " + rv);
                String autostart = getAnnotation(buildConfig, AUTOSTART);
                if (Boolean.parseBoolean(autostart)) {
                    logger.info("Auto-starting job " + jobName + " from BuildConfig " + ns + " with revision: " + rv);
                    job.scheduleBuild2(0);
                }
            } catch (IllegalArgumentException e) {
                // see https://github.com/openshift/jenkins-sync-plugin/issues/117, jenkins
                // might reload existing jobs on newJob check above and when we make the
                // createProjectFromXML call; if so, retry as an update
                logger.warning("Exception while creating job from xml: " + e.getMessage());
                updateJob(job, jobStream, existingBuildRunPolicy, buildConfigProjectProperty);
                logger.fine("Updated job " + jobName + " from BuildConfig " + ns + " with revision: " + rv);
            }
        } else {
            updateJob(job, jobStream, existingBuildRunPolicy, buildConfigProjectProperty);
            logger.fine("Updated job " + jobName + " from BuildConfig " + ns + " with revision: " + rv);
